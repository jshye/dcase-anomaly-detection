...
def main(config):
    ...
    for idx, target_dir in enumerate(dir_list):
        ...
        print("================ BUILD MODEL =================")
        model = AEBaseline(x_dim=config[machine_type]['n_mels']*config[machine_type]['n_frames'], h_dim=128, z_dim=8, n_hidden=3).to(DEVICE)
        ...
        for epoch in range(1, 1+config['training']['epochs']):
            ...
            with tqdm(data_loader['train'], ncols=100) as pbar:
                for x, s, y in pbar:  # data, section id, anomaly
                    ...
                    out = model(x)
                    train_loss_step = criterion(out, x.squeeze())
            ...
            with torch.no_grad():
                with tqdm(data_loader['val'], ncols=100) as pbar:
                    for x, s, y in pbar:  # data, section id, anomaly
                        ...
                        val_loss_step = criterion(out, x.squeeze())
            ...
            with open(csv_logdir, 'a') as f:
                f.write(f'{epoch},{train_loss},{val_loss},_\n')
